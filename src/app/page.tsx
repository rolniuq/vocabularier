...
type ViewMode = 'daily' | 'flashcard' | 'quiz' | 'writing' | 'stats' | 'browse' | 'add' | 'achievements' | 'settings' | 'oxford';
...
export default function Home() {
  ...
  const {
   ...
   const handleKeyboardNav = useCallback((tabIndex: number) => {
     const tabs: ViewMode[] = ['daily', 'flashcard', 'quiz', 'writing', 'stats', 'browse', 'oxford'];
   ...
   useKeyboardShortcuts([
     ...
     { key: '7', action: () => handleKeyboardNav(6), description: 'Go to Oxford 3000' },
  ...
  const handleImportWords = (importedWords: Word[]) => {
   ...
   const renderContent = () => {
     switch (viewMode) {
       ...
       case 'oxford':
         return <OxfordPractice onClose={() => setViewMode('daily')} />;
   ...
   const tabs = [
     ...
     { id: 'oxford', label: 'Oxford 3000', icon: '📖' },
   ];
